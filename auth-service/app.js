...
module.exports = async () => {
  ...
  await require('./migration').run().then(() => {
  ...
  app.use((req, res, next) => {
    ...
    req.logId = [SERVICE_NAME, `traceId[${req.traceId}]`, `spanId[${req.id}]`, `user[${req.header('user.id') ? (req.header('user.id') + ',' + req.header('user.type')) : '-'}]`].join(' ');
    req.log = (...args) => {
  ...
  app.use(morgan(`:timestamp ${SERVICE_NAME} traceId[:traceId] spanId[:id] user[:userId] :method :url :status :response-time ms`, { stream: logger.stream }));
  ...
  if ('production' === process.env.NODE_ENV) {
    app.use('/hospital-iam/api/v1/api-docs', notFoundHandler);
  } else {
    logger.info(`Swagger link: http://localhost:${process.env.PORT}/hospital-iam/api/v1/api-docs `)
  }

  app.use('/hospital-iam/api/v1/', express.static(path.join(__dirname, 'public')));
  ...
  app.use(function (error, req, res, next) {
    if (req.error) {
      ...
      logger.error(`${new Date().toISOString()} ${req.logId} error handler: :${JSON.stringify(error)}, obj: ${error}`);
    }
